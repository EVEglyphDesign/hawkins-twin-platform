...
MD = HERE / "README.md"
OUT = HERE / "WARRANTY-GENE-SCHEMA.pdf"
...
@page {{
  ...
  @top-left {{
    content: "EVEglyphDesign — Warranty GENE Schema";
    color: #e87722;
...
html_body = re.sub(
    r"^<h1>Warranty GENE — VIN Rich-Target Schema \(v1\)</h1>\s*<p>([^<]+)</p>",
    lambda m: (
        '<div class="title-block">'
        '<div class="kicker">EVEglyphDesign · Hawkins Twin Platform</div>'
        '<h1>Warranty GENE — Schema v1</h1>'
        '<div class="subtitle">VIN rich-target row, coverage math, and sample fixtures</div>'
        f'<p>{m.group(1)}</p>'
        '</div>'
    ),
    html_body, count=1,
...
<html><head><meta charset="utf-8"><title>Warranty GENE — Schema</title></head>
<body>{html_body}</body></html>"""

(HERE / "WARRANTY-GENE-SCHEMA.html").write_text(HTML_DOC, encoding="utf-8")
