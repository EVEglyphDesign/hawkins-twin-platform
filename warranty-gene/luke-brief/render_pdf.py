"""Render EgD-WGB-007 Luke Live Brief markdown to canon PDF via WeasyPrint."""
...
MD = HERE / "EgD-WGB-007-Luke-Live-Brief.md"
OUT = HERE / "EgD-WGB-007-Luke-Live-Brief.pdf"
...
def md_to_html(md_text: str) -> str:
    ...
    while i < len(lines):
        ...
        if "|" in ln and ln.strip().startswith("|"):
            close_lists()
            if not in_table: in_table = True; tbl_rows = []
...
@page {{
  ...
  background: #fdfaf4;
  @top-left {{
    content: "EVEglyphDesign — Warranty GENE Luke Live Brief (EgD-WGB-007)";
    color: #e87722; font-family: Inter, sans-serif; font-weight: 600;
    font-size: 8.5pt; padding-bottom: 4mm;
    border-bottom: 0.6pt solid #e87722;
  }}
  @top-right {{
    content: "{KEY}  ·  {ts}";
    color: #6b665c; font-family: Inter, sans-serif; font-size: 8pt;
    padding-bottom: 4mm; border-bottom: 0.6pt solid #e87722; white-space: nowrap;
  }}
  @bottom-left {{
    content: "© 2026 EVEglyphDesign  ·  SHA-256 {sha[:10]}…";
    color: #6b665c; font-family: Inter, sans-serif; font-size: 7.5pt;
    padding-top: 3mm; border-top: 0.4pt solid #e7e1d3; white-space: nowrap;
  }}
  @bottom-right {{
...
html, body {{ background: #fdfaf4; color: #1a1a1a;
  font-family: Inter, "Helvetica Neue", Arial, sans-serif;
  font-size: 10.5pt; line-height: 1.55; hyphens: auto; }}
body::before {{ content: ""; position: fixed; top: 40%; left: 15%;
  transform: rotate(-24deg); color: #f2ebd8;
  font-family: "Fraunces", "Times New Roman", serif; font-size: 96pt;
  font-weight: 400; letter-spacing: -1pt; z-index: -1; opacity: 1; }}
...
li {{ margin: 0 0 3pt; }}
blockquote {{ margin: 8pt 0 12pt 8pt; padding: 4pt 12pt;
  border-left: 2pt solid #e87722; color: #6b665c; font-style: italic; }}
...
table.data {{ width: 100%; border-collapse: collapse; margin: 6pt 0 14pt;
  font-size: 9pt; }}
table.data th {{ text-align: left; background: #f7f2e7;
  border-bottom: 0.8pt solid #e87722; padding: 5pt 7pt;
  font-family: Inter, sans-serif; font-weight: 700; color: #1a1a1a; }}
table.data td {{ border-bottom: 0.3pt solid #e7e1d3; padding: 4pt 7pt;
  vertical-align: top; }}
table.data tr {{ page-break-inside: avoid; }}
.title-block {{ margin-bottom: 20pt; }}
.title-block .kicker {{ font-family: Inter, sans-serif; font-size: 9pt;
  text-transform: uppercase; letter-spacing: 2pt; color: #e87722;
  margin-bottom: 4pt; }}
.title-block h1 {{ font-size: 34pt; line-height: 1.05; margin: 0; }}
.title-block .subtitle {{ font-family: "Fraunces", serif; font-size: 14pt;
  color: #6b665c; margin: 4pt 0 10pt; font-style: italic; }}
.closing {{ text-align: center; color: #6b665c; font-style: italic;
            margin-top: 20pt; font-size: 9pt; }}
"""

html_body = re.sub(
    r"^<h1>Warranty GENE — Luke Live Brief</h1>\s*<p>(.+?)</p>",
    lambda m: (
        '<div class="title-block">'
        '<div class="kicker">EVEglyphDesign · Hawkins Twin Platform</div>'
        '<h1>Warranty GENE — Luke Live Brief</h1>'
        '<div class="subtitle">EgD-WGB-007 · Proving Luke\'s DMS logins pull real data · One hour to demo</div>'
        f'<p>{m.group(1)}</p>'
        '</div>'
    ),
...
<html><head><meta charset="utf-8"><title>Warranty GENE — Luke Live Brief (EgD-WGB-007)</title></head>
<body>{html_body}</body></html>"""

(HERE / "EgD-WGB-007-Luke-Live-Brief.html").write_text(HTML_DOC, encoding="utf-8")
